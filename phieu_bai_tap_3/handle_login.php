<?php 
// TODO 1: (Cực kỳ quan trọng) Khởi động session // Phải gọi hàm này TRƯỚC BẤT KỲ output HTML nào 
...
// Gợi ý: Dùng hàm isset() để kiểm tra sự tồn tại của $_POST['username']  
    ...
    if ($user == 'minhtu' && $pass == '03012005') {
        // Nếu $user == 'minhtu' VÀ $pass == '03012005' thì là đăng nhập thành công
        // TODO 5: Nếu thành công, lưu tên username vào SESSION 
        $_SESSION['minhtu'] = $user; 
 
        // TODO 6: Chuyển hướng người dùng sang trang "chào mừng" 
        header('Location: welcome.php'); 
        exit; 
    } else if ($user == 'guest' && $pass == 'guestpass') {
    // Nếu $user == 'admin' VÀ $pass == '123' thì là đăng nhập thành công     if ($user == 'admin' && $pass == '123') { 
         
        // TODO 5: Nếu thành công, lưu tên username vào SESSION 
        
        // TODO 6: Chuyển hướng người dùng sang trang "chào mừng" 
        // Gợi ý: Dùng hàm header('Location: ...');         // Và luôn gọi exit() ngay sau khi dùng header()         exit;          
    } else { 
        // Nếu thất bại, chuyển hướng về login.html 
        header('Location: login.html?error=1'); // Kèm theo thông báo lỗi trên URL         exit; 
    } 
} else {   
